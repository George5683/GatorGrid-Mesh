...
err_t tcp_server_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p,
  ...
  // Reset the TCP buffer state or else data will just be appened to the state
  // recv buffer
  state->recv_len = 0;
...
bool APNode::server_running() { return !this->state->complete; }
...
APNode::APNode()
    : state(nullptr), running(false), password("password"), rb(10),
      tree(get_NodeID()), Master_Pico() {
  snprintf(ap_name, sizeof(ap_name), "GatorGrid_Node:%08X", get_NodeID());
  Master_Pico.SPI_init(true);
}

APNode::APNode(uint32_t id)
    : state(nullptr), running(false), password("password"), rb(10), tree(id),
      Master_Pico() {
  set_NodeID(id);
  snprintf(ap_name, sizeof(ap_name), "GatorGrid_Node:%08X", get_NodeID());
  Master_Pico.SPI_init(true);
...
struct data APNode::digest_data() { return this->rb.digest(); }

int APNode::number_of_messages() { return this->rb.get_size(); }
...
int APNode::get_node_id() { return get_NodeID(); }

// MeshNode class function for getting Node ID
uint32_t MeshNode::get_NodeID() { return NodeID; }
...
void APNode::server_start() { run_tcp_server(state); }
...
MeshNode::~MeshNode() { NodeID = 0; }

bool APNode::send_tcp_data(uint32_t id, tcp_pcb *client_pcb, uint8_t *data,
                           uint32_t size) {
  ...
  // err_t err = tcp_write(state->client_pcb, (void*)buffer, size,
  // TCP_WRITE_FLAG_COPY); err_t err2 = tcp_output(state->client_pcb);
...
bool APNode::handle_incoming_data(unsigned char *buffer, tcp_pcb *tpcb,
                                  struct pbuf *p) {
  bool ACK_flag = true;
  bool NAK_flag = false;
  bool update_flag = false;
  uint8_t error = 0;
  // tcp_init_msg_t *init_msg_str = reinterpret_cast <tcp_init_msg_t
  // *>(state->buffer_recv); clients_map[tpcb].id = init_msg_str->source;
  ...
  TCP_MESSAGE *msg =
      parseMessage(reinterpret_cast<uint8_t *>(state->buffer_recv));
  uint8_t msg_id = 0xFF;

  if (!msg) {
    DEBUG_printf(
        "Error: Unable to parse message (invalid buffer or unknown msg_id).\n");
    ACK_flag = false;
  } else {
    ...
    switch (msg_id) {
        ...
        DEBUG_printf("Testing dataMsg, len:%u, source:%08x, dest:%08x\n",
                    dataMsg->msg.msg_len, dataMsg->msg.source,
                    dataMsg->msg.dest);
        if (dataMsg->msg.dest == get_NodeID()) {
            rb.insert(dataMsg->msg.msg, dataMsg->msg.msg_len, dataMsg->msg.source,
                    dataMsg->msg.dest);
            ...
            //     TCP_NAK_MESSAGE nakMsg(get_NodeID(), clients_map[tpcb].id,
            //     p->tot_len); nakMsg.set_error(error);
            //     send_tcp_data(dataMsg->msg.source,
            //     client_tpcbs.at(dataMsg->msg.source), nakMsg.get_msg(),
            //     nakMsg.get_len()); break;
            // }
            ...
            // NAK_flag = true;
            error =
                0x02; // TODO make enum for errors (id not in connected clients)
            TCP_NAK_MESSAGE nakMsg(get_NodeID(), clients_map[tpcb].id,
                                    p->tot_len);
            ...
            send_tcp_data(dataMsg->msg.dest, client_tpcbs.at(dataMsg->msg.dest),
                        dataMsg->get_msg(), dataMsg->get_len());
        }
        break;
        }
        case 0x02: {
        ...
        if (ackMsg->msg.dest == get_NodeID()) {
            ...
            TCP_NAK_MESSAGE nakMsg(get_NodeID(), clients_map[tpcb].id,
                                    p->tot_len);
            nakMsg.set_error(error);
            send_tcp_data(ackMsg->msg.source, client_tpcbs.at(ackMsg->msg.source),
                            nakMsg.get_msg(), nakMsg.get_len());
            break;
            }
            ACK_flag = false;
            send_tcp_data(ackMsg->msg.dest, client_tpcbs.at(ackMsg->msg.dest),
                        ackMsg->get_msg(), ackMsg->get_len());
            }
        ...
        case 0xFF: {
            uint32_t children[4];
            memcpy(collect_children().data(), children, sizeof(children));

            // Need to send message to STA to send to this node's parent
            //send_tcp_data();

            break;
        }
        default:
            DEBUG_printf("Error: Unable to parse message (invalid buffer or unknown "
                        "msg_id).\n");
            ACK_flag = false;
            break;
        }
  ...
  if (update_flag) {
    ...
    // Because this is a new node, resend the list of connected nodes to
    everyone for(auto i : client_tpcbs) { ids[j] = i.first; j++;
    }
    ...
    for(auto i : client_tpcbs) {
        sleep_ms(20);
        send_tcp_data(i.first, i.second, updateMsg.get_msg(),
    updateMsg.get_len()); DEBUG_printf("Sent update message to %08x\n",
    i.first);
    }*/
  }
  ...
  if (ACK_flag) {
    ...
    // Assumption: clients_map[tpcb].id implies that any message worth acking
    // isn't being forwarded and is originating from the intended node
    TCP_ACK_MESSAGE ackMsg(get_NodeID(), clients_map[tpcb].id, p->tot_len);
    send_tcp_data(clients_map[tpcb].id, tpcb, ackMsg.get_msg(),
                  ackMsg.get_len());
    ...
    nakMsg.set_error(error);
    send_tcp_data(clients_map[tpcb].id, tpcb, nakMsg.get_msg(),
                  nakMsg.get_len());
...
std::vector<uint32_t> APNode::collect_children() {
  std::vector<uint32_t> children;

  for (auto child : client_tpcbs) {
